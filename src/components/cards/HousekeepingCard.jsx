...
const HousekeepingCard = ({ heading, title, src }) => {
  const { setShowCart, selectServices, HousekeepingAssistance, setRequestPopup, selectRequestPopupData } =
  ...
  const handleSelectedServices = (heading, title) => {
    ...
    if (title === "In-Room Dining" || title === "Buffet") {
      ...
      const service = HousekeepingAssistance.find(
        (service) => service.subtitle === heading && service.items.some(item => item.title === title)
      );
  ...
  return (
    ...
    >
      <div onClick={() => handleSelectedServices(heading, title)} className="relative w-full aspect-square bg-tertiary rounded-xl">
        <img
